...
def inference(
    ...
    num_workers: int = 4,
    batch_size: int = 8,
    color_norm_fp: str = "utils/normalization.npy",
) -> np.ndarray:
    ...
    Args:
        img_dir (str): The directory containing the tile images.
        model (str): Filepath to gray / white matter model.
        img_size (int, optional): The size of the tile images. Defaults to 1536.
        stride (int, optional): The stride of the sliding window. Defaults to 16.
        num_workers (int, optional): The number of workers to use for inference. 
            Defaults to 4.
        batch_size (int, optional): The batch size for inference. Defaults to 8.
        color_norm_fp (str, optional): The file path to the color normalization 
            parameters. Defaults to "utils/normalization.npy".
            
    Returns:
        np.ndarray: The inference mask.
    ...
    seg_output = np.zeros(
        (heatmap_res * row_nums, heatmap_res * col_nums), dtype=np.uint8
    )
    ...
    for idx in tqdm(indices, desc="Inferencing on each tile:"):
        ...
        image_datasets = HeatmapDataset(
            img_dir, row, col, normalize, stride=stride, img_size=img_size
        )
